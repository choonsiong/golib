...
func TestGetTimeInTimezone(t *testing.T) {
	...
	tests := []struct {
		...
		{"Asia/London", time.Now(), "Asia/London", time.Time{}, ErrInvalidTimezone},
		{"Asia/Tokyo", time.Now(), "Asia/Tokyo", time.Now().In(tokyoLoc), nil},
		{"Empty", time.Time{}, "", time.Time{}, ErrInvalidTimezone},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := GetTimeInTimezone(tt.t, tt.loc)

			if tt.wantErr != nil {
				if err == nil {
					t.Errorf("GetTimeInTimezone(%v, %v) == nil; want %v", tt.t, tt.loc, tt.wantErr)
				}

				if !errors.Is(err, tt.wantErr) {
					t.Errorf("GetTimeInTimezone(%v, %v) == %v; want %v", tt.t, tt.loc, err, tt.wantErr)
				}
			}

			if got.Format("2006-01-02 15:04:05") != tt.want.Format("2006-01-02 15:04:05") {
				t.Errorf("GetTimeInTimezone(%v, %v) == %v; want %v", tt.t, tt.loc, got, tt.want)
			}
		})
	}
}
